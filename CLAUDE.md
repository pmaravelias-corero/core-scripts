...
| Path | Purpose |
|---|---|
| `core-local-deploy/core-local-deploy.sh` | Start or tear down one CORElet project locally |
| `core-sync-projects/core-sync-projects.sh` | Batch-sync all CORElet repos to their remote default branch |
| `core-mcp/core_mcp.py` | Read-only MCP server that gives Claude Code ambient knowledge of the workspace |
| `core-mcp/install.sh` | Installs the MCP Python package and registers `core-mcp` with Claude Code |
| `core-dev-hub/index.html` | Local browser dashboard (CORE Dev Hub) with tabbed views; tab logic lives in `core-dev-hub/tabs/` |

## Key lists to keep in sync

Both shell scripts contain a hardcoded list that must be updated when a new CORElet is added:

- `CORELETS` in `core-local-deploy/core-local-deploy.sh` — drives shell tab-completion only; does not affect runtime behaviour
- `PROJECTS` in `core-sync-projects/core-sync-projects.sh` — determines which repos are synced
