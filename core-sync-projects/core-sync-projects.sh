...
prune_merged_branches() {
  ...
  if [[ -z "$merged" ]]; then
    ok "No merged local branches to prune"
    return
  fi
...
# Delete local branches whose upstream tracking ref is gone (e.g. squash-merged PRs).
# Uses -D because squash-merges rewrite history, so the branch tip is not reachable from HEAD.
prune_gone_branches() {
  info "Pruning gone-upstream local branches..."
  local gone
  gone=$(git branch -vv \
    | grep ': gone\]' \
    | sed 's/^\* //' \
  ...
  if [[ -z "$gone" ]]; then
    ok "No gone-upstream branches to prune"
    return
  fi
  ...
  ok "Gone-upstream branches pruned"
}

# Show git status for a single project.
process_status() {
  local proj="$1"
...
echo
if [[ "$MODE" == status ]]; then
  echo -e "${BOLD}Status of ${#PROJECTS[@]} projects in ${PROJECTS_DIR}${RESET}"
else
